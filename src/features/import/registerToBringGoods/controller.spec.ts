...
import ImportRegisterToBringGoodsController from './controller';
...
describe(`[GET] ${Route.importRegisterToBringGoods}`, () => {
  it('It should respond with statusCode 200', async () => {
    await request(app.getServer())
      .get(`${Route.importRegisterToBringGoods}${getParams(params)}`)
      .set('user-agent', 'node-superagent')
      .expect(200)
      .then((res) => {
        expect(res.text).toContain('Check which services you need to register with');
      });
  });
  ...
  it('It should throw an error and call next', async () => {
    const createMockRequest = () => (
      {
        ...
        route: {
          path: Route.importRegisterToBringGoods,
        },
        csrfToken: () => 'csrftoken',
    ...
    const controller = new ImportRegisterToBringGoodsController();
    await controller.importRegisterToBringGoods(mockRequest, mockResponse, mockNext);
