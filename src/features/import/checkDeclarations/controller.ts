...
class ImportCheckDeclarationsController {
   public importCheckDeclarations: RequestHandler = (req, res, next) => {
     ...
     const previousPage = `${journey.import.importCheckDeclarations.previousPage()}?${res.locals.queryParams}`;
     const commodityClassification = hierarchy(findCode(commodity));

     try {
       res.render('import/checkDeclarations/view.njk', {
         previousPage,
         tradeDate,
         commodity,
         additionalCode,
...
export default ImportCheckDeclarationsController;
