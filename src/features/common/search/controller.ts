...
class Search {
   ...
   public search: RequestHandler = async (req, res, next) => {
     ...
     const invalidDate = validateImportDate(importDate, translation, String(tradeType), language);
     ...
     const additionalCodeRoute = Route.additionalCode;

     clearSessionErrorMessages(req);

     try {
       if (commodityCodeIsEmpty(commodity)) {
         redirectRoute(searchRoute, isEdit ? `${queryParams}&isEdit=true` : queryParams, res, req, translation.page.importGoods.errors.required);
         ...
         if (this.isValidCommodityCode(response)) {
           ...
           if (hasNoAdditionalCodes(additionalCodeRadios)) {
             return redirectRoute(Route.checkYourAnswers, removeParam(newQueryParams.toString(), ['additionalCode', 'original']), res);
           }
         ...
         if (this.isChapter(response)) {
           ...
           searchResultsRows = await getResultsRowsChapters(items.data, commodity, queryParams, translation);
         ...
         if (this.isCommodity(response)) {
           ...
           searchResultsRows = await getResultsRows(groupByTypeFiltered, commodity, queryParams, translation, language);
         ...
         const breadcrumbItems = await getBreadcrumbs([], commodity, queryParams, sid, translation, '', language);
