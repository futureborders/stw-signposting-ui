...
import AdditionalCodeController from './controller';
...
describe(`[POST] ${Route.additionalCode}`, () => {
  it(`It should respond with statusCode 302 when posted and redirect to ${Route.checkYourAnswers}`, async (done) => {
    const axiosAdditionalCodeResponse: AxiosResponse = {
      ...mockedAdditionalCodeData,
      status: 200,
      statusText: 'OK',
    ...
    await request(app.getServer())
      .post(`${Route.additionalCode}${getParams(params)}`)
      .set('user-agent', 'node-superagent')
      .set('Cookie', csrfResponse.cookies)
      .send(data)
      .expect(302, {})
      .expect('Location', `${Route.checkYourAnswers}${getParams(params)}&additionalCode=${data.additionalCode}`);
  });
  ...
  it('It should return status 500', async () => {
    ...
    const controller = new AdditionalCodeController(mockedStwTradeTariffApi);
    await controller.additionalCodeSubmit(mockRequest, mockResponse, mockNext);
