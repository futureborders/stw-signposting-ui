...
class ExportCheckInformationAndDocumentsController {
  public checkInformationAndDocuments: RequestHandler = (req, res, next) => {
    const {
      commodity,
      destinationCountry,
      originCountry,
    ...
    try {
      setSessionExport(req, calculateNewTaskStatuses(getSessionExport(req), ExportsTasks.checkInformationAndDocuments, TaskStatus.VIEWED));

      const previousPage = journey.export.checkInformationAndDocuments.previousPage();
      ...
      res.render('export/checkInformationAndDocuments/view.njk', {
        previousPage,
        tradeDate,
        commodityCode,
        additionalCode,
...
export default ExportCheckInformationAndDocumentsController;
