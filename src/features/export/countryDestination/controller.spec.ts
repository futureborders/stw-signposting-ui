...
describe(`[POST] ${Route.exportCountryDestination}`, () => {
  ...
  it(`It should respond with statusCode 302 and redirect to ${Route.exportCheckYourAnswers} when isEdit`, async (done) => {
    ...
    await request(app.getServer())
      ...
      .send(data)
      .expect(302, {})
      .expect('Location', `${Route.exportCheckYourAnswers}?tradeType=export&exportDeclarations=yes&originCountry=GB&destinationCountry=CN&checkWhatServicesYouNeedToRegister=true`);
  });
});
