...
class ExportCheckDeclarationsController {
   public exportCheckDeclarations: RequestHandler = (req, res, next) => {
     ...
     try {
       setSessionExport(req, calculateNewTaskStatuses(getSessionExport(req), ExportsTasks.checkCustomsDeclarations, TaskStatus.VIEWED));

       const previousPage = journey.export.exportCheckDeclarations.previousPage();
